...
function templates_table() {
	global $d_root,$d_atemplate;
	$gui=new ScriptedUI();
	$gui->add("form","adminform","","admin.php?com_option=admintemplates");
	$gui->add("com_header",_TEMPLATES_HEAD);

	$table_head = array ( array('title'=>'radio' , 'val'=>'id' , 'len'=>'1%') ,
						  array('title'=>_NAME,'val'=>'name','len'=>'40%','info'=>_INFO,'ilink'=>'admin.php?com_option=admintemplates&task=info&cid[]=ivar1','ivar1'=>'id') ,
	...
	$table_data = read_dir_xml($d_root."admin/templates/","template.xml",$table_req, 'admintemplates/admintemplate');
	$table_data = insert_published($table_data,"published",$d_atemplate);
...
function edit_template($template) {
	...
	$html_file=$d_root."admin/templates/".$template."/index.php";
	$css_file1=$d_root."admin/templates/".$template."/css/template.style.css";
	$css_file2=$d_root."admin/templates/".$template."/css/ui.style.css";

	$gui=new ScriptedUI();
	$gui->add("form","adminform","","admin.php?com_option=admintemplates");
	$gui->add("hidden","atemplate","",$template);
	...
	$gui->add("tab_end");
	$gui->add("tab",_TEMPLATES_EDIT_CSS,writable_string($css_file1),"dtab");
	//$gui->add("hidden","css_file","",$css_file,"");
	$gui->add("textarea_big","template_css_data",_TEMPLATES_EDIT_CSS,xhtml_safe(file_get_contents($css_file1)));
	$gui->add("tab_end");
	$gui->add("tab",_TEMPLATES_EDIT_CSS_UI,writable_string($css_file2),"dtab");
	$gui->add("textarea_big","template_css_data_ui",_TEMPLATES_EDIT_CSS_UI,xhtml_safe(file_get_contents($css_file2)));
	$gui->add("tab_end");
	$gui->add("tab_tail");
	$gui->add("end_form");
...
function info_template($template) {
	...
	$xml_file=$d_root."admin/templates/".$template."/template.xml";
	$preview_path=$d_subpath."admin/templates/".$template."/template_thumbnail.png";
	...
	$info=read_file_xml($xml_file,$table_req,'admintemplates/admintemplate');
	if (!$info) {
		global $pathway;
