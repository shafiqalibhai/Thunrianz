 ...
 */
define('__DOMPDF_FONT_CACHE_FILE', DOMPDF_FONT_DIR . "dompdf_font_family.cache");
...
class Font_Metrics {
  ...
  static function get_default_font() {
  ...
  static function save_font_families() {
	$data = self::$_font_lookup;
    //legolas558: render paths as relative
    $l = strlen(DOMPDF_FONT_DIR);
    foreach(array_keys($data) as $font) {
	foreach(array_keys($data['font']) as $type) {
		$data[$font][$type] = substr($data[$font][$type], $l);
	}
    }
...
//      return;
    ...
    //legolas558: render paths as absolute
    foreach(array_keys($data) as $font) {
	foreach(array_keys($data[$font]) as $type) {
		$data[$font][$type] = DOMPDF_FONT_DIR . $data[$font][$type];
	}
    }
    self::$_font_lookup = $data;
