...
// this is necessary on hosts with adoDB compiled-in extension, see http://bugs.laniuscms.org/view.php?id=753
if (!defined('ADODB_FETCH_DEFAULT')) {
...
function &ADONewConnection( $dbtype = 'mysql', $modules = '' )
{
	...
	/**
	 * Count uses of each generic module. Useful for creating unique class names
	 */
	static $generic_module_uses = array();
	...
	include_once ADODB_DIR . '/adodb_sql_drivers/' . $dbtype . '/' . $dbtype . '_driver.inc';
	...
	if(!empty($modules))
	{
		...
		foreach($module_list as $mod) {
			...
			{
				include_once ADODB_DIR . '/adodb_sql_drivers/' . $dbtype . '/' . $dbtype . '_' . $mod . '_module.inc';
		...
		if(count($generic_modules))
		{
			foreach($generic_modules as $mod) {
	            if (array_key_exists($mod, $generic_module_uses))
	            {
					// this module already used, make a new one based on it
					$new_mod = $mod . $generic_module_uses[$mod];
					$generic_module_uses[$mod]++;
					_ADOCreateUniqueMod($mod, $new_mod, $last_module);
					$last_module = $new_mod;
	            }
	            else
	            {
					include ADODB_DIR . '/generic_modules/' . $mod . '_module.inc';
					$generic_module_uses[$mod] = 1;
					$last_module = $mod;
	            }
			}
		}
...
/**
 * Create new classes based on a generic module file. This ugly hack is a
 * workaround for the v1.42 bug described in the link:
 * @link http://sourceforge.net/tracker/?func=detail&atid=747945&aid=1680204&group_id=140982
 * @author mrclay
 *
 * Note: Since the module files use their own eval-ed extender classes,
 * rather than double-evaling, we could rewrite those out of the code and have
 * the new classes directly extend our "last module" classes, but this would
 * probably be more trouble than it's worth and make the generic module code
 * more fragile.
 *
 * @access private
 * @param string $mod module name
 * @param string $new_mod the module to be "created"
 * @param string $last_module
  */
function _ADOCreateUniqueMod($mod, $new_mod, $last_module)
{
	$code = file_get_contents(ADODB_DIR . '/generic_modules/' . $mod . '_module.inc');
	...
	// Avoid redeclaring "empty" classes such as ADOFetchObj in object_module.inc
	$code = preg_replace('/class\s+\w+\s*{\s*}/', '', $code);
	// $last_module passed in so it will be in local scope during eval execution
	eval($code);
}
...
function &NewDataDictionary(&$connection, $dbtype=false)
{
	...
	include_once ADODB_DIR . '/adodb_sql_drivers/' . $dbtype . '/' . $dbtype . '_datadict.inc';
