...
function timezones_array($selected = null) {
	global $timezones;
	$tz_arr = array( array('name' => '-- Auto --', 'value' => '') );
	$tzs = array_keys($timezones);
	foreach($tzs as $timezone) {
		$b = ($timezone === $selected);
		$timezone = xhtml_safe($timezone);
		$a = array('name' => $timezone, 'value' => $timezone);
		if ($b) $a['selected'] = true;
		$tz_arr[] = $a;
	}
	return $tz_arr;
}
